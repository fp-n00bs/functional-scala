...
object algebra {
  ...
  object First {
    implicit def FirstSemigroup[A]: Semigroup[First[A]] =
      new Semigroup[First[A]] {
        def append(l: First[A], r: => First[A]): First[A] =
          l
      }
  }
  ...
  def TryMonoid[A: Semigroup]: Monoid[scala.util.Try[A]] =
    new Monoid[scala.util.Try[A]] {
      ...
      def append(l: Try[A], r: => Try[A]): Try[A] =
        (l, r) match {
          case (Failure(el), Failure(er)) => Failure(el)
          case (Failure(el), Success(r))  => Success(r)
          case (Success(l), Failure(er))  => Success(l)
  ...
  def MonoidMap[K, V: Semigroup]: Monoid[Map[K, V]] =
    new Monoid[Map[K, V]] {
      ...
      def append(l: Map[K, V], r: => Map[K, V]): Map[K, V] =
        (l.keySet ++ r.keySet).flatMap { key =>
          val newValue = l.get(key) |+| r.get(key)
          newValue map (v => key -> v)
        }.toMap
    }
  ...
  case class UserPermission(value: List[(AccountID, ResourceID, Capability)]) {

    /**
     * Provides the set of all resources the user has access to.
     */
    def allResources: Set[ResourceID] = value.map(_._2).toSet
    ...
    def capabilitiesFor(resourceID: ResourceID): Set[Capability] =
      value.filter(_._2 == resourceID).map(_._3).toSet
    ...
    def audit(resourceID: ResourceID, capability: Capability): Set[AccountID] =
      value.filter(v => v._2 == resourceID && v._3 == capability).map(_._1).toSet
  }
...
//  val example2 = mzero[UserPermission] |+| UserPermission(???)
  ...
  implicit def MonoidNotEmpty[A: Monoid]: Monoid[NotEmpty[A]] = new Monoid[NotEmpty[A]] {
    ...
    override def append(f1: NotEmpty[A], f2: => NotEmpty[A]): NotEmpty[A] =
      NotEmpty(f1.head |+| f2.head, (f1.tail, f2.tail) match {
...
object functor {
  ...
  val EitherToFuture: Either[Throwable, ?] ~> scala.concurrent.Future =
    new NaturalTransformation[Either[Throwable, ?], scala.concurrent.Future] {
      def apply[A](fa: Either[Throwable, A]): scala.concurrent.Future[A] = fa match {
        case Left(ex) => scala.concurrent.Future.failed(ex)
        case Right(a) => scala.concurrent.Future.successful(a)
      }
    }
  ...
  val ZipFuture: Zip[scala.concurrent.Future] =
    new Zip[scala.concurrent.Future] {
      ...
      def zip[A, B](l: Future[A], r: Future[B]): Future[(A, B)] =
        l.flatMap(resL => r.map(resR => (resL, resR)))
    }
  ...
  val OptionApplicative: Applicative[Option] =
    new Applicative[Option] {
      ...
      def zip[A, B](fa: Option[A], fb: Option[B]): Option[(A, B)] =
        fa.flatMap(resL => fb.map(resR => (resL, resR)))
  ...
  implicit def zip[F[_]: Applicative, A, B](l: F[A], r: F[B]): F[(A, B)] =
    Applicative[F].apply2(l, r)((a, b) => Tuple2(a, b))
  implicit def ap2[F[_]: Applicative, A, B](fa: F[A], fab: F[A => B]): F[B] =
